...
public class PatternStore {
  ...
  private List<Pattern> mPatterns = new LinkedList<>(); // mPatterns is a list of all patterns in the store (order unspecified);
  private Map<String,List<Pattern>> mMapAuths = new HashMap<>(); //mMapAuths is a map from author names to lists of Patterns from that author (one author can be associated with multiple distinct patterns)
  private Map<String,Pattern> mMapName = new HashMap<>();//mMapName is a map from names to Pattern objects.
  ...
  public PatternStore(String source) throws IOException {
   ...
   private void load(Reader r) throws IOException {
      // DONE: read each line from the reader and print it to the screen
      /*
